...
(function(){
  ...
  // v30: weather baseline is fixed to Daejeon Metropolitan City, not Seoul/device location.
  // Open-Meteo is used without an API key. A cached value keeps the game stable when offline.
  const WEATHER_BASE={name:'대전',lat:36.3504,lng:127.3845};
  const WEATHER_CACHE_KEY='jopams_weather_daejeon_v1';
  ...
  function cachedEnvironment(){try{const x=JSON.parse(localStorage.getItem(WEATHER_CACHE_KEY)||'null');if(x&&x.at&&Date.now()-x.at<2*60*60*1000)return x.env}catch(e){}return null}
  function environment(){const cached=cachedEnvironment();if(cached)return cached;const h=new Date().getHours(),night=h<6||h>=19;return {period:night?'night':'day',weather:'clear',location:WEATHER_BASE.name,temperature:null,source:'fallback'}}
  function applyEnvironment(env){document.documentElement.dataset.period=env.period;document.documentElement.dataset.weather=env.weather;const weatherName={rain:'비',cloud:'구름',clear:'맑음'}[env.weather]||'맑음';document.querySelectorAll('[data-env-label]').forEach(el=>el.textContent=`${WEATHER_BASE.name} · ${env.period==='night'?'야간':'주간'} · ${weatherName}`);if(document.body?.classList.contains('ar-body')){let layer=document.querySelector('.ambient-layer');if(!layer){layer=document.createElement('div');layer.className='ambient-layer';layer.innerHTML='<i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i>';document.body.appendChild(layer)}}window.dispatchEvent(new CustomEvent('jopams:environment',{detail:env}))}
  async function refreshEnvironment(force=false){const cached=cachedEnvironment();if(cached&&!force){applyEnvironment(cached);return cached}const url=`https://api.open-meteo.com/v1/forecast?latitude=${WEATHER_BASE.lat}&longitude=${WEATHER_BASE.lng}&current=weather_code,is_day,temperature_2m&timezone=auto`;try{const res=await fetch(url,{cache:'no-store'});if(!res.ok)throw new Error('weather '+res.status);const data=await res.json(),cur=data.current||{};const env={period:Number(cur.is_day)===0?'night':'day',weather:weatherKind(cur.weather_code),location:WEATHER_BASE.name,temperature:Number.isFinite(Number(cur.temperature_2m))?Number(cur.temperature_2m):null,source:'open-meteo'};localStorage.setItem(WEATHER_CACHE_KEY,JSON.stringify({at:Date.now(),env}));applyEnvironment(env);return env}catch(e){const env=environment();applyEnvironment(env);return env}}
